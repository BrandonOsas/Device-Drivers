...
*/
/*--------------- LCD Messages ---------------*/
const char* const MSG_HOST_INIT      = "> Host Library Initialized\n";
const char* const MSG_DEV_ATTACHED   = "> Device Attached \n";
const char* const MSG_DEV_DISCONNECTED   = "> Device Disconnected\n";
const char* const MSG_DEV_ENUMERATED     = "> Enumeration completed \n";
const char* const MSG_DEV_HIGHSPEED      = "> High speed device detected\n";
const char* const MSG_DEV_FULLSPEED      = "> Full speed device detected\n";
const char* const MSG_DEV_LOWSPEED       = "> Low speed device detected\n";
const char* const MSG_DEV_ERROR          = "> Device fault \n";

const char* const MSG_MSC_CLASS     = "> Mass storage device connected\n";
const char* const MSG_HID_CLASS     = "> HID device connected\n";
const char* const MSG_DISK_SIZE     = "> Size of the disk in MBytes: \n";
const char* const MSG_LUN           = "> LUN Available in the device:\n";
const char* const MSG_ROOT_CONT     = "> Exploring disk flash ...\n";
const char* const MSG_WR_PROTECT    = "> The disk is write protected\n";
const char* const MSG_UNREC_ERROR   = "> UNRECOVERED ERROR STATE\n";
...
void USBH_USR_Init(void)
{
  ...
  if(startup == 0 )
  {
    ...
    LCD_LOG_SetFooter ((uint8_t*)"     USB Host Library v2.0.0" );
  }
}
...
void USBH_USR_DeviceDisconnected (void)
{
  ...
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, (uint8_t*)"                                   ");
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, (uint8_t*)"                                   ");
...
void USBH_USR_EnumerationDone(void)
{
  ...
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, (uint8_t*)"To see the root content of the disk : " );
  LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, (uint8_t*)"Press Key...                       ");
...
int USBH_USR_MSC_Application(void)
{
  ...
  case USH_USR_FS_WRITEFILE:
    ...
    LCD_SetTextColor(Green);
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, (void*)"                                              ");
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, (void*)"Press Key to write file");
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR);
    ...
    LCD_SetTextColor(Green);
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, (void*)"                                              ");
    LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, (void*)"To start Image slide show Press Key.");
    LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR);
    break;
  ...
  if (res == FR_OK) {
    while(HCD_IsDeviceConnected(&USB_OTG_Core))
    {
      ...
      if(line_idx > 9)
      {
        ...
        LCD_SetTextColor(Green);
        LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 42, (void*)"                                              ");
        LCD_DisplayStringLine( LCD_PIXEL_HEIGHT - 30, (void*)"Press Key to continue...");
        LCD_SetTextColor(LCD_LOG_DEFAULT_COLOR);
...
static uint8_t Image_Browser (char* path)
{
  ...
  LCD_LOG_SetHeader(" USB OTG HS MSC Host");
#else
  LCD_LOG_SetHeader((void*)" USB OTG FS MSC Host");
#endif
  LCD_LOG_SetFooter ((void*)"     USB Host Library v2.0.0" );
  LCD_UsrLog("> Disk capacity : %ld Bytes\n", USBH_MSC_Param.MSCapacity * \
      USBH_MSC_Param.MSPageLength);
  USBH_USR_ApplicationState = USH_USR_FS_READLIST;
  return ret;
